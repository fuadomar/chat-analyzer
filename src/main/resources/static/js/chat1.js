function createChatList(userId) {
  var chatListDiv = '<div id="chat-' + userId + '">  <h3>' + userId
      + ' :</h3> <ol id="messages"></ol>'
      + ' <div class="panel-footer"><div class="' + '"input-group">'
      + '<input id="+message_' + userId + '" ' + 'class="btn-input" '
      + 'type="text" ' + 'class="form-control input-sm chat_input" ' +
      'placeholder="Write your message here..."/>' + ' </div></div></div>';

  chatListDiv = ' <div class="user" id="user' + userId + '">' + userId
      + '</div>';
...
$(document).ready(function () {
  ...
  stompClient.connect('', function (frame) {
    ...
    stompClient.subscribe("/topic/chat.message" + "-" + sessionId,
        function (data) {
          ...
          $('#chatbox-container').on('customOnMessageEvent',
              "#msg_push" + payload.sender, function (event, msg) {

                msg = payload.sender + ": " + msg;
                if (msg != '') {
                  $('<div class="msg_a">' + msg + '</div>').insertBefore(
                      '#msg_push' + payload.sender);
                }
                $('#msg_body' + payload.sender).scrollTop(
                    $('#msg_body' + payload.sender)[0].scrollHeight);
                event.stopImmediatePropagation();
              });
          $("#msg_push" + payload.sender).trigger('customOnMessageEvent',
              [payload.message]);
        });
